...
class gallery {
	...
	public static $dir = '../upload/albums/';
	...
	public static function addGallery() { // 2016-05-16
		...
		if (empty($response['errors'])) {
			...
			if (!empty($gallery_id)) {
				...
				switch($translates){
					case !empty($translates['en']['name']): $dirname=$translates['en']['name'];  break;
					case !empty($translates['ru']['name']): $dirname=$translates['ru']['name'];  break;
					case !empty($translates['az']['name']): $dirname=$translates['az']['name'];  break;
				}
				
				mkdir(self::$dir.$dirname);
				
				// saving translates

				foreach ($translates as $lang=>$tr_data) {

					$fieldname = key($tr_data);
	...
	public static function getAutocomplete($q) { // 2016-05-10
