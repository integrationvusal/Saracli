...
class nav {
	...
	public static $menu_tbl = 'menu';

	public static $tr_fields = ['name', 'is_published_lang'];
	public static $types = ['category','url','phonebook','personalities', 'gallery'];
	...
	private static function checkSef(&$response, &$item, $item_id=0) { // 2016-05-11
		...
		} else if (self::isSefExists($sef, $item_id)) {
	...
	private static function processNavItemPositions(&$response, &$positions) { // 2016-05-11
        return true; // in this project we have got only one menu type - main menu
	...
	public static function getNavItem($id) { // 2016-05-11
		$sql = "SELECT n.*, cu.name AS author_name, mcu.name AS editor_name
			FROM menu n
				LEFT JOIN cms_users cu ON cu.id=n.add_by
	...
	public static function addNavItem() { // 2016-05-25
		...
		if (empty($response['errors'])) {
			...
			$item['ref_id'] = abs((int)$_POST['article_id']);
	...
	public static function showNavTree($nav, $current, $link_sc) { // 2016-05-11
		static $time;
		...
		if (!empty($nav) && is_array($nav)) {
			...
			foreach ($nav as $n) {
				$html.='<li data-jstree="'.utils::safeEcho(json_encode([
						...
						'icon' => 'nav-role-'.$n['type'],
					]), 1).'"'.($n['is_published']? '': ' class="nav-node-unpublished"').'><a href="'.$link_sc.'&amp;page='.$current.'&amp;item='.$n['id'].'" data-item_id="'.$n['id'].'" data-sef-url="'.$n['sef'].'">'.$n['name'].'</a>'.(empty($n['children'])? '': self::showNavTree($n['children'], $current, $link_sc)).'</li>';
			}
			$html.='</ul>';
		}
		return $html;
	...
	public static function setNavItemPosition($items){
		foreach($items as $id=>$ordering){
			CMS::$db->mod('menu#'.$id, [
				'ordering' => abs((int)$ordering)
			]);
		}
		return true;
	}

	public static function setNavItemParent($parent) { // 2016-05-12
		$id = abs((int)key($parent));
		$parent = abs((int)$parent[$id]);
	...
	public static function getCats($parent=false) { // 2016-06-07
		/*$cats = [];
