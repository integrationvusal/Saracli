...
class news {
	...
	private static function checkGallery(&$response, &$news, $news_id=0) { // 2016-05-17
		$news['gallery_id'] = '0';
	...
	public static function getNewsList() { // 2016-12-04
		$list = [];
	...
	public static function addNews() { // 2016-06-10
		...
		if (!utils::valid_date(@$_POST['publish_date'])) {
			...
			$news['publish_datetime'] = utils::formatPlainDate('Y-m-d', $_POST['publish_date']).' '.str_pad((int)$_POST['publish_hour'], 2, STR_PAD_LEFT).':'.str_pad((int)$_POST['publish_minutes'], 2, STR_PAD_LEFT).':00';
		}
		...
		if (!utils::valid_date(@$_POST['add_date'])) {
			...
			$news['add_datetime'] = utils::formatPlainDate('Y-m-d', $_POST['add_date']).' '.str_pad((int)$_POST['add_hour'], 2, STR_PAD_LEFT).':'.str_pad((int)$_POST['add_minutes'], 2, STR_PAD_LEFT).':00';
		}


		$sef = trim(@$_POST['sef']);
		$againSef = CMS::$db->getRow("SELECT id FROM ".self::$tbl." WHERE sef = :sef", ['sef'=>$sef]);
		if(!empty($againSef))	$response['errors'][] = 'news_add_err_sef_again';
		...
		if (!empty($_FILES['img']['name'])) {
			if (empty($_FILES['img']['error'])) {
				$news['img'] = utils::upload($_FILES['img']['name'], $_FILES['img']['tmp_name'], UPLOADS_DIR.'news/originals/', self::$allowed_thumb_ext);
				if (empty($news['img'])) {
					$response['errors'][] = 'upl_invalid_image_extension_err';
				} else {
					foreach (self::$dimensions as $dir=>$size) {
						@mkdir(UPLOADS_DIR.'news/'.$dir.'/', 0777, true);
						$img = new SimpleImage(UPLOADS_DIR.'news/originals/'.$news['img']);
						$img->thumbnail($size['width'], $size['height'])->save(UPLOADS_DIR.'news/'.$dir.'/'.$news['img']);
					}
				}
			} else {
	...
	public static function editNews($id) { // 2016-06-10
		...
		$sef = utils::makeSEF($sef);
		if (empty($sef)) {
			$response['errors'][] = 'news_edit_err_sef_empty';
		} else {
			$upd['sef'] = $sef;
		}
		...
		if (!empty($_FILES['img']['name'])) {
			if (empty($_FILES['img']['error'])) {
				$uploaded = utils::upload($_FILES['img']['name'], $_FILES['img']['tmp_name'], UPLOADS_DIR.'news/originals/', self::$allowed_thumb_ext);
				if (empty($uploaded)) {
					$response['errors'][] = 'upl_invalid_image_extension_err';
				} else {
					$upd['img'] = $uploaded;
					@unlink(UPLOADS_DIR.'news/originals/'.$news['img']);

					foreach (self::$dimensions as $dir=>$size) {
						@unlink(UPLOADS_DIR.'news/'.$dir.'/'.$news['img']);
						$img = new SimpleImage(UPLOADS_DIR.'news/originals/'.$uploaded);
						$img->thumbnail($size['width'], $size['height'])->save(UPLOADS_DIR.'news/'.$dir.'/'.$uploaded);
					}
				}
			} else {
				$response['errors'][] = CMS::$upload_err[$_FILES['img']['error']];
		...
		if (!utils::valid_date(@$_POST['publish_date'])) {
			...
			$upd['publish_datetime'] = utils::formatPlainDate('Y-m-d', $_POST['publish_date']).' '.str_pad((int)$_POST['publish_hour'], 2, STR_PAD_LEFT).':'.str_pad((int)$_POST['publish_minutes'], 2, STR_PAD_LEFT).':00';
		}
		...
		if (!utils::valid_date(@$_POST['add_date'])) {
			...
			$upd['add_datetime'] = utils::formatPlainDate('Y-m-d', $_POST['add_date']).' '.str_pad((int)$_POST['add_hour'], 2, STR_PAD_LEFT).':'.str_pad((int)$_POST['add_minutes'], 2, STR_PAD_LEFT).':00';
		}
