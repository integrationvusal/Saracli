...
class news_controller extends controller {
	...
	public static function action_edit() { // 2016-01-15
		self::$layout = 'common_layout';
		view::$title = CMS::t('menu_item_news_edit');

		$params = [];

		$params['canWrite'] = CMS::hasAccessTo('news/edit', 'write');
		$params['link_back'] = (empty($_GET['return'])? '?controller=news&action=list': $_GET['return']);
		...
		return self::render('news_edit', $params);
	}
	...
	public static function action_ajax_sort() { // 2016-12-05
		...
		if (!CMS::hasAccessTo('news/ajax_sort', 'write')) {
			$response['code'] = '403';
	...
	public static function action_ajax_paged_sort() { // 2016-12-05
		...
		if (!CMS::hasAccessTo('news/ajax_paged_sort', 'write')) {
			$response['code'] = '403';
			$response['message'] = 'ajax_request_not_alllowed_to_write';
		} else if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])=='xmlhttprequest') {
	...
	public static function action_ajax_delete_image() { // 2016-12-05
		...
		if (!CMS::hasAccessTo('news/ajax_delete_image', 'write')) {
			$response['code'] = '403';
			$response['message'] = 'ajax_request_not_alllowed_to_write';
		} else if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])=='xmlhttprequest') {
