...
<section class="contextual-navigation">
	<nav>
		...
		<a href="?controller=news&amp;action=add&amp;return=<?=$link_return;?>&amp;<?=time();?>" class="btn btn-default"><i class="fa fa-plus-circle" aria-hidden="true"></i> <?=CMS::t('menu_item_news_add');?></a>
		<?php } ?>
	</nav>
</section>
...
<section class="content">
	...
	<div class="box">
		<div class="box-header with-border">
			...
			<select name="filter[cats][]" class="fast-filter-cats" form="formSearchAndFilter">
				<option value=""><?=CMS::t('filter_all')?></option>
				<?php foreach ($cats as $c) {	if (!empty($allowed_cats) && !in_array($c['id'], $allowed_cats)) continue; ?>
					<option value="<?=$c['id']?>" <?=@in_array($c['id'], $_GET['filter']['cats'])?'selected="selected"':null?> ><?=$c['name']?></option>
				<?php } ?>
			</select>
				...
				<tbody>
					...
					?>
						<tr data-id="<?=$a['id'];?>" data-ordering="<?=$a['ordering'];?>">
							...
							<td>
								<?php if (CMS::hasAccessTo('news/ajax_set_status', 'write')) { ?>
								<a href="?controller=news&amp;action=ajax_set_status" title="" class="aAjax btn-toggle" data-ajax_post="<?=utils::safeEcho(json_encode([
									'CSRF_token' => $CSRF_token,
									'id' => $a['id'],
							...
							<td style="white-space: nowrap;">
								...
								<a class="a_i" href="?controller=news&amp;action=edit&amp;id=<?=$a['id'];?>&amp;return=<?=$link_return;?>&amp;<?=time();?>" title="<?=CMS::t('edit');?>">
								...
								<?php if (CMS::hasAccessTo('news/delete', 'write') && !isset($_GET['see_deleted'])) { ?>
...
<div id="popupFilter" style="display: none; width: 420px;">
	...
	<div class="popupForm">
		<div class="popupFormFieldset">
			...
			<div class="popupFormInputsBlock">
				<label for="selectAssignment" class="form-label"><?=CMS::t('news_assignment');?></label>

				<select name="filter[assignment]" id="selectAssignment" class="form-control" form="formSearchAndFilter">
					<option value=""><?=CMS::t('filter_all');?></option>
					<option value="on_main_page"<?=((@$_GET['filter']['assignment']=='on_main_page')? ' selected="selected"': '');?>><?=CMS::t('news_show_on_main_page_filter');?></option>
					<option value="off_main_page"<?=((@$_GET['filter']['assignment']=='off_main_page')? ' selected="selected"': '');?>><?=CMS::t('news_show_off_main_page_filter');?></option>
					<option value="highlighted"<?=((@$_GET['filter']['assignment']=='highlighted')? ' selected="selected"': '');?>><?=CMS::t('news_is_highlighted_filter');?></option>
				</select>
			</div>
			...
			<div class="popupFormInputsBlock">
				<label for="selectAssignment" class="form-label"><?=CMS::t('news_type');?></label>

				<div class="form-div-multicheckbox" style="width: 100%; height: auto; max-height: 214px;">
					<?php foreach ($types as $c) {
							$is_selected = isset($_GET['filter']['doc_types']) && in_array($c['id'], $_GET['filter']['doc_types']) ? 'checked="checked"': null;?>

					<input form="formSearchAndFilter" type="checkbox" name="filter[doc_types][]" value="<?=$c['id'];?>" <?=$is_selected?> id="multiCheckboxCat_<?=$c['id'];?>" /><label for="multiCheckboxCat_<?=$c['id'];?>"> <?=$c['type'];?></label><br />
			...
			<div class="popupFormInputsBlock">
				<label class="form-label"><?=CMS::t('news_category');?></label>
				<div class="form-div-multicheckbox">
					<?php if (empty($allowed_cats)) { ?>
					<input type="checkbox" name="filter[cats][none]" value="1"<?=(isset($_GET['filter']['cats']['none'])? ' checked="checked"': '');?> id="multichoiceCat_None" form="formSearchAndFilter" /><label for="multichoiceCat_None"> <?=CMS::t('news_category_none');?></label><br />
					<?php } ?>
					<?php
						foreach ($cats as $c) {
