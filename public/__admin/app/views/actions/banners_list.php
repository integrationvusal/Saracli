...
$(document).ready(function() {
	...
	$('#tablesorter').sortable({
		...
		update: function(e, ui){
			...
			console.log(_cal_start);
			console.log(_items)

			$.ajax({
				url: "<?=view::create_url('banners', 'ajax_sort')?>",
				async: false,
				cache: false,
				method: 'POST',
				data: {
...
// ]]>
	...
	<form action="<?=view::create_url('banners', 'delete') ?>" method="post" id="formDeleteItem">
	...
	<section class="contextual-navigation">
		<nav>
			<?php if (CMS::hasAccessTo('banners/add', 'write')) { ?>
			<a href="<?=view::create_url('banners', 'add', ['return' => $link_return]) ?>&amp;<?=time();?>" class="btn btn-default"><i class="fa fa-plus-circle" aria-hidden="true"></i> <?=CMS::t('menu_item_banners_add');?></a>
			<?php } ?>
		</nav>
	</section>
	...
	<section class="content">
		...
		<div class="box">
			<div class="box-header with-border">
				<h3 class="box-title"><?=CMS::t('banners_list_details', [
					'{count}' => $count,
				...
				<div class="box-tools pull-right col-sm-5 col-lg-6">
					<form action="" method="get" id="formSearchAndFilter">
						...
						<div class="input-group has-feedback">
							<input type="text" name="q" value="<?=utils::safeEcho(@$_GET['q'], 1);?>" placeholder="<?=CMS::t('banners_search');?>" class="form-control input-md" onfocus="this.value='';" />
            ...
            <div class="box-body">
				...
				<table class="table table-bordered table-striped">
					<thead>
						<tr>
							<th><?=CMS::t('banners_link');?></th>
							<th><?=CMS::t('banners_img');?></th>
					...
					<tbody id="tablesorter" data-page="<?=$current-1?>" data-limit="<?=$limit?>">
						...
						?>
							<tr data-id="<?=$data['id']?>">
								<td><?=$data['link']?></td>
								<td><img width="100px" src="<?=SITE?>/upload/banners/<?=$data['img']?>"/></td>
								<td>
									<?php if (CMS::hasAccessTo('banners/edit', 'write')) { ?>
										<a href="<?=view::create_url('banners', 'edit', ['id'=>$data['id']])?>" title="<?=CMS::t('edit');?>">
											<i class="fa fa-pencil-square-o" aria-hidden="true"></i>
										</a>
									<?php } ?>
									
									<?php if (CMS::hasAccessTo('banners/delete', 'write')) { ?>
